...
const ProductList = () => {
  const { searchQuery, selectedCategory, pageNumber, setPageNumber } = useProducts();
    ...
    const loadProducts = async () => {
      try {
        ...
        if (searchQuery) {
          ...
          data = await fetchProductsByCategoryId(selectedCategory, pageNumber, 10, "productName", "desc");
        } else {
      ...
      <div className="space-y-6">
        {products.map((product) => (
          <div key={product.productId} className="flex items-center space-x-6">
            <img src={product.image} alt={product.productName} className="w-24 h-24 object-cover" />
            <div className="flex flex-col">
              <h3 className="text-xl font-semibold">{product.productName}</h3>
              <p className="text-sm text-gray-700 mb-2">{product.description}</p>
              <div className="text-lg font-bold">${product.specialPrice.toFixed(2)}</div>
